...
def diffusion_adjoint_solve(solver_params, print_interval=100):
    """
    ...
    D_consts = [Constant(float(D_vals[i])) for i in range(n_species)]
    ...
    for step in range(num_steps):
        ...
        try:
            solve(F_res == 0, U, bcs=bcs, J=J, solver_parameters=params)
        except Exception as e:
            print(f"Failed to converge: {e}")
            break
        U_prev.assign(U)

    return U_prev, z_vals, D_consts, mesh, V_scalar
