...
def build_forms(ctx,solver_params):
    ...
    # Controls you want to infer
    #D = [fd.Function(V_scalar).assign(D_vals[i]) for i in range(n)]
    # D = [fd.Constant(D_vals[i]) for i in range(n)] 

    #For some ungodly reason, firedrake will only optimize a function
    #Here we create a function space with a single point
    # R = fd.FunctionSpace(mesh, "R", 0)
    # D = [fd.Function(R, name=f"D{i}") for i in range(n)]
    # for i in range(n):
    #     D[i].assign(float(D_vals[i]))

    R = fd.FunctionSpace(mesh, "R", 0)  # one global DOF
    m = [fd.Function(R, name=f"logD{i}") for i in range(n)]
    for i in range(n):
        m[i].assign(np.log(float(D_vals[i])))  # initialize logD from D_vals

    D = [fd.exp(m[i]) for i in range(n)]     # UFL expression, always positive
