...
I'm not sure how this would work with a weak formulation based forward method. To my understanding this involves spacially descritizing the PDE, ie going from
...
to
\begin{equation}
    \frac{d \bm{u}}{d t} = \bm{F}(\bm{u})
\end{equation}
by letting each row of $\bm{u}$ be the value of $u$ at a spacial gridpoint. We now have a system of ODEs which we can reduce by using a kind of spectral analysis to figure out a reduced order basis for this new system.\\
...
In the immidiate future I plan to explore using firedrake to create a Newton-CG solver that works with a simpler poisson system. I can't seem to find any existing implementations of Newton-CG with the firedrake framework.
