...
\begin{document}
\section*{A brief summary of inverse problems}
...
\end{equation}
But we first need to find the right search direction $\bm{p}_k$ that will affect $\bm{m}_k$ in the right way. We do this by solving the system
...
I'd like to know a bit more about the applications. If the goal is to create a desired $c_i(\bm{x},T_f), \phi(\bm{x},T_f)$, and we have some mechanisms for controlling the system, then an inverse problem with the constraints as the control mechanism might have a better shot and not being quite so ill-conditioned.
...
I may explore using firedrake to create a Newton-CG solver. I can't seem to find any existing implementations of Newton-CG with the firedrake framework.
